...
public class KpiReportExporter implements ReportExporter {
	...
	private final String USER_LOGIN = "KPI System";
	...
	public void exportExcel(final String destination, final String processDate) throws Exception {
		...
		try {
			msigListLots = getMsigListlots(yyyyMM);
			logger.info("MSIG WB List Lots >> " + msigListLots);

			addDataToKpiResult(yyyyMM, msigListLots);
...
//		criteria.add(Restrictions.eq("endDate", endDate));
	...
	private String getMsigListlots(String processDate) throws Exception {
		String result = "";
		...
		for(KpiCategorySetup k : list) {
			if(!result.contains(k.getListLotCode())) {
				if(StringUtils.isEmpty(result)) {
					result += k.getListLotCode();
				} else {
					result += "," + k.getListLotCode();
				}
			}
	...
	private void addDataToKpiResult(String yearMonth, String fixListlots) throws Exception {
...
//		List<KpiResult> list = getKpiResultByYearMonth(yearMonth);
//		for(KpiResult k : list) {
//			kpiResultService.delete(k);
//		}
...
//		<!-- Add to Kpi Result -->
		logicKpiResult(kpiBeanList, yearMonth, fixListlots);
	}
